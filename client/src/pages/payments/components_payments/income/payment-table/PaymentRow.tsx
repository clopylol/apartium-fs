            ...
            <td className="px-6 py-4">
                {payment.status === 'paid' ? (
                    <div className="flex flex-col">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-ds-success/20 text-ds-success border border-ds-success/40 w-fit shadow-sm"><CheckCircle className="w-3 h-3" /> {t('payments.table.status.paid')}</span>
                        <span className="text-[10px] text-ds-success mt-1 pl-1 font-medium font-mono">{payment.date}</span>
                    </div>
                ) : (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-ds-destructive/20 text-ds-destructive border border-ds-destructive/40 shadow-sm"><XCircle className="w-3 h-3" /> {t('payments.table.status.unpaid')}</span>
                )}
            </td>
            <td className="px-6 py-4 text-right">
                <div className="flex items-center justify-end gap-3">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <div className={`relative w-11 h-6 rounded-full transition-all duration-200 ${payment.status === 'paid' ? 'bg-ds-success' : 'bg-ds-destructive'} peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-offset-2 peer-focus:ring-offset-ds-background-dark ${payment.status === 'paid' ? 'peer-focus:ring-ds-success' : 'peer-focus:ring-ds-destructive'}`}>
