...
interface DuesGeneratorModalProps {
    ...
    selectedYear: string;
    onClose: () => void;
    onGenerate: (mode: 'single' | 'bulk', amount: number, bulkList?: any[]) => void;
}
    ...
    const footer = (
        <div className="flex gap-3">
            ...
            <button
                ...
                className={`flex-1 py-3 rounded-xl font-bold text-sm transition-colors shadow-lg ${
                    duesMode === 'single' && isSelectedPeriodPast
                        ? 'bg-[#1A1D26] text-slate-500 cursor-not-allowed'
                        ...
                        {isSelectedPeriodPast ? (
                            ...
                            <>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">{t('payments.modals.dues.labels.amount')}</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-[#151821] border border-white/10 rounded-xl pl-8 pr-3 py-3 text-sm text-white focus:outline-none focus:border-[#3B82F6] transition-colors font-bold"
                                        />
                                    </div>
                                </div>
                            </>
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                        <div className="bg-ds-in-indigo-light/10 border-2 border-ds-in-indigo-light/20 p-4 rounded-xl flex items-end gap-4 shadow-lg shadow-ds-in-indigo-light/5">
                            <div className="flex-1 space-y-1.5">
                                <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">{t('payments.modals.dues.labels.defaultAmount')}</label>
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-ds-in-indigo-light font-bold">₺</span>
                                    <input
                                        type="number"
                                        value={duesAmountInput}
                                        onChange={(e) => setDuesAmountInput(parseInt(e.target.value))}
                                        className="w-full bg-[#151821] border border-white/10 rounded-xl pl-8 pr-3 py-2 text-sm text-white focus:outline-none focus:border-[#3B82F6] transition-colors font-bold"
                                    />
                                </div>
                            </div>
                            <button
                                onClick={handleApplyBulkAmount}
                                className="px-4 py-2 bg-[#3B82F6] hover:bg-[#2563EB] text-white rounded-lg text-sm font-bold transition-all shadow-lg shadow-blue-500/20 hover:scale-105 mb-0.5"
                            >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-h-[60vh] overflow-y-auto custom-scrollbar p-1">
                            {bulkDuesList.map((item, index) => {
                                ...
                                return (
                                    <div key={`${item.month}-${item.year}`} className={`bg-ds-background-dark/50 border-2 rounded-xl p-4 flex flex-col gap-3 transition-all group ${
                                        isLocked 
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className={`w-full bg-[#151821] border border-white/10 rounded-xl pl-6 pr-2 py-2 text-sm text-white focus:outline-none focus:border-[#3B82F6] font-medium text-right transition-colors ${
