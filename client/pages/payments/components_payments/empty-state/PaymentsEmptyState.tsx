...
export const PaymentsEmptyState: FC<PaymentsEmptyStateProps> = ({ onGenerateDues }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center py-16 animate-in fade-in duration-500">
            <div className="w-20 h-20 bg-ds-card-dark rounded-full flex items-center justify-center mb-6 ring-1 ring-ds-border-dark shadow-inner">
                <Wallet className="w-10 h-10 text-ds-muted-light opacity-50" />
            </div>
            ...
            <p className="text-ds-muted-light mb-8 leading-relaxed text-center max-w-md">
                {t('payments.emptyState.description')}
            </p>
            {onGenerateDues && (
                <Button
                ...
                >
                    {t('payments.emptyState.generateButton')}
                </Button>
            )}
        </div>
    );
